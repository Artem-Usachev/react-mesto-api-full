...
const likeCard = (req, res, next) => {
  Card.findByIdAndUpdate(req.params.card._id, { $addToSet: { likes: req.user._id } }, { new: true })
    .then((cardLiked) => {
      if (!cardLiked) {
        throw new NotFoundError('Карточка не найдена');
...
const dislikeCard = (req, res, next) => {
  Card.findByIdAndUpdate(req.params.card._id, { $pull: { likes: req.user._id } }, { new: true })
    .then((cardDisliked) => {
      if (!cardDisliked) {
        throw new NotFoundError('Карточка не найдена');
